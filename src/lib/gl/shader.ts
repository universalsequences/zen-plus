...
`;
  ...
  let functions = "";
  const functionsEmitted = new Set<string>();
  if (out.functions) {
    for (let fn of out.functions) {
      if (functionsEmitted.has(fn.code)) {
        continue;
      }
      functions += printFunction(context, fn);
      functionsEmitted.add(fn.code);
    }
  }
...
const printFunction = (context: Context, fn: Generated): string => {
  ...
  if (fn.functionArguments) {
    code += fn.functionArguments
      .sort((a, b) => a.num - b.num)
      .map((arg) => `${context.printType(arg.type)} ${arg.name}`)
      .join(",");
  }
  code += `) {
${fn.code}
