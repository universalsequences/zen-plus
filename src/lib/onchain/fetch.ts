...
export const fetchRevisions = async (
  ...
  const revs: Revision[] = [{ name: json.name, tokenId }];
...
export const fetchOnchainSubPatch = async (
  publicClient: PublicClient,
  tokenId: number,
): Promise<SerializedPatch> => {
  const uri = await fetchTokenURI(publicClient, tokenId);
  const diff = uri.diff;

  const decompressed = decompress(diff);
  return decompressed;
};
...
const fetchTokenURI = async (publicClient: PublicClient, tokenId: number): Promise<any> => {
  ...
  const json = base64ToJson(data as string);
  return json;
};
...
export const decompress = (compressed: any): any => {
  ...
  const _json = JSON.parse(decompressed);
  return _json;
};
