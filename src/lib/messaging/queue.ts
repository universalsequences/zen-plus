...
const MessageQueue = () => {
  ...
  const publish = (type: string, ..._messages: Message[]) => {
    if (!messages[type]) {
      messages[type] = [];
    }
    messages[type].unshift(_messages[0]);
    if (messages[type].length > 2) {
      messages[type].pop();
    }
    notify(type, ..._messages);
  };

  const notify = (type: any, ...messages: Message[]) => {
    if (subscribers[type]) {
      subscribers[type].forEach((fn) => fn(...messages));
    }
  };
  ...
  const clearQueue = () => {
    ...
    for (let sub of badSubscribers) {
      for (let type in subscribers) {
        subscribers[type] = subscribers[type].filter((fn) => fn !== sub);
      }
    }
