...
${genBlockArrays(graph.codeBlocks)}

EMSCRIPTEN_KEEPALIVE
void process(float * inputs, float * outputs) {
${genCodeBlocks(graph, graph.codeBlocks)}
...
const genSIMDArrays = (simdBlocks: SIMDBlock[]): string => {
    console.log('gen simd arrays=', simdBlocks);
    let code = "";
...
`;
    ...
    console.log('simd arrays=', code);
    return code;
}

const genBlockArrays = (scalarBlocks: CodeBlock[]): string => {
    let code = "";
    for (let block of scalarBlocks) {

        let variables = Array.from(new Set([...block.variables!, block.variable]));
...
const genCodeBlocks = (graph: ZenGraph, simdBlocks: CodeBlock[]): string => {
    sortDependencies(simdBlocks);
    let code = "";
    for (let block of simdBlocks) {
        if (block.isSIMD) {
            let variable = block.variable;
...
${prettyPrint("        ", block.code)}
`;
            let variables = Array.from(new Set([...block.variables!, block.variable]));
            for (let variable of variables) {
                if (block.code.includes("v128_t " + variable)) {
                    code += `        wasm_v128_store(block_${variable} + j, ${variable}); // Store the result
`;
                } else {
                }
            }
...
`;
            ...
            let variable = block.variable;
            code += `    for (int j=0; j < BLOCK_SIZE; j++) {
        ${genInputs(graph)}
        ${declareOutputs(graph)}
        ${genHistories(graph)}
        ${genBlockHistories(block.histories)}
${prettyPrint("        ", block.code)}
        block_${variable}[j] = ${variable}; // Store the result
}
`;
        }
    };
...
const sortDependencies = (blocks: CodeBlock[]): CodeBlock[] => {
    for (let i = 0; i < blocks.length; i++) {
        for (let j = 0; j < blocks.length; j++) {
            if (i < j) {
                if (blocks[i].variables!.includes(blocks[j].variable!)) {
                    console.log("swapping two blocks", blocks[i], blocks[j], i, j);
                    let tmp = blocks[i];
                    blocks[i] = blocks[j];
                    blocks[j] = tmp;
                }
...
const genBlockHistories = (his: string[]): string => {
    let hist = his.join("\n");
    hist = hist.replace("let", "float") + ';';
    console.log('block histories...', hist);
    return hist;
};
