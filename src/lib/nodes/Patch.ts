...
export class PatchImpl implements Patch {
  ...
  recompileGraph() {
    ...
    let _objectNodes =
      this.name === undefined ? this.getAllNodes() : objectNodes; //objectNodes;
    ...
    if ((this as Patch as SubPatch).parentPatch) {
      ...
      for (let node of objectNodes) {
        if (
          node.subpatch &&
          node.subpatch.patchType === OperatorContextType.GL
        ) {
          node.subpatch.recompileGraph(true);
        }
      }
    ...
    let matricesAndBuffers = this.objectNodes.filter(
    ...
    let sourceNodes = objectNodes.filter(
    ...
    if (this.isZenBase()) {
      let audioInputs = this.getAllNodes()
        .filter((node) => node.name === "in")
        .filter((node) => !(node.patch as SubPatch).parentPatch.isZen);
      audioInputs.forEach((input) => input.receive(input.inlets[0], "bang"));
      ...
      if (parent) {
        ...
        functions.forEach((x) => x.receive(x.inlets[0], "bang"));
      }
    }
    ...
    if (this.isZenBase()) {
      // || this.name === undefined) {
      let histories = this.getAllNodes().filter(
        (node) => node.name === "history",
      );
      histories.forEach((sourceNode) => {
        if (sourceNode.fn) {
          sourceNode.receive(sourceNode.inlets[0], "bang");
        }
      });
      let args = this.getAllNodes().filter(
        (node) => node.name === "argument" || node.name === "invocation",
      );

      args.forEach((sourceNode) => {
        if (sourceNode.fn) {
          sourceNode.receive(sourceNode.inlets[0], "bang");
        }
      });
    ...
    if (this.name === undefined) {
      sourceNodes = this.getAllNodes().filter(
        (node) => node.name === "uniform",
      );
      sourceNodes.forEach((sourceNode) => {
        if (sourceNode.fn) {
          sourceNode.receive(sourceNode.inlets[0], "bang");
    ...
    sourceNodes = objectNodes.filter(
      (node) => node.name === "data" && node.inlets[0].connections.length === 0,
    );
    sourceNodes.forEach((sourceNode) => {
      if (sourceNode.fn) {
    ...
    sourceNodes = _objectNodes.filter(
    ...
    if (this.isZenBase()) {
      ...
      let calls = this.getAllNodes().filter(
        (node) =>
          node.operatorContextType === OperatorContextType.ZEN &&
          (node.name === "call" ||
            node.name === "latchcall" ||
  ...
  compile(statement: Statement, outputNumber?: number) {
    if (outputNumber !== undefined) {
      ...
      let numFound = this.outputStatements.filter(
        (x) => x !== undefined,
      ).length;
    ...
    setTimeout(() => {
      ...
      if (this.historyDependencies.length > 0) {
        let historyDependencies = this.historyDependencies.filter((x) =>
          notInFunction(x),
        );
        ...
        let __statement: Statement[] = sortHistories(
      ...
      let a = new Date().getTime();
      let zenGraph: ZenGraph = Array.isArray(ast)
        ? zen(...ast)
        : zen(ast as UGen);
      ...
      createWorklet(this.audioContext, zenGraph, workletId)
        .then((ret) => {
          ...
          let merger = this.audioContext.createChannelMerger(
          ...
          if (parentNode) {
            ...
            let allReceives = root
              .getAllNodes()
              .filter((x) => x.name === "receive~");
  ...
  fromJSON(x: SerializedPatch, isPreset?: boolean): Connections {
    ...
    for (let serializedNode of x.objectNodes) {
      ...
      if (name === "in" || name === "out") {
        ...
        let _objectNode = this.objectNodes.find(
    ...
    let loadBangs = this.objectNodes.filter(
  ...
  setupPostCompile(useDeep = false) {
    ...
    let o = useDeep ? this.getAllNodes() : this.objectNodes;
    let loadBangs = o.filter(
      (x) => x.operatorContextType === OperatorContextType.CORE && x.needsLoad,
    );
    loadBangs.forEach((x) => x.receive(x.inlets[0], "bang"));
  }
  ...
  resolveMissedConnections() {
    for (let [connection, source, dest, outletNumber] of this
      .missedConnections) {
...
const getFunctionNames = (dslCode: string, ultraMinify = true) => {
  ...
  Object.keys(functions).forEach((func) => {
    ...
    /*
        ...
        */
    let shorthand =
      alphabet[prefixIndex] +
      (suffixIndex > 0 ? alphabet[suffixIndex - 1] : "");
...
function sortDependencies(pairs: DependencyPair[]): DependencyPair[] {
  ...
  const allDependenciesSorted = (
    dependencies: any[],
    sorted: DependencyPair[],
  ): boolean => {
  ...
  while (sorted.length < pairs.length) {
    ...
    if (!addedToSortedThisRound && sorted.length < pairs.length) {
      throw new Error(
        "Cannot sort due to unsatisfiable dependencies or a dependency cycle.",
      );
    }
  }
