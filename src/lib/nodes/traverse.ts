import { Node, ObjectNode, Patch, SubPatch, IOConnection } from './types';
...
const expandConnections = (connection: IOConnection): IOConnection[] => {
    ...
    if (subpatch) {
        ...
        let inObject = subpatch.objectNodes.find(x => x.name === "in" && x.arguments[0] === inletNumber + 1);
...
export const traverseForwards = (node: Node, originalNode: Node = node, visited: Set<Node> = new Set<Node>(), debug = false): Node[] => {
    let ins: Node[] = [node];
    visited.add(node);
    for (let outlet of node.outlets) {
        let _connections = outlet.connections.flatMap(x => expandConnections(x));
