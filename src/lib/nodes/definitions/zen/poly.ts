...
export const polycall = (node: ObjectNode, ...args: Lazy[]) => {
  ...
  return (message: Message) => {
    ...
    const _args = args.map((x) => x()).filter((x) => x !== undefined);
    const voices = node.attributes.voices as number;

    const numBodies = (body as Statement[]).length - 1;

    const mode = node.attributes.mode;
    const outputs =
      mode !== "pipe" ? new Array(numBodies).fill(0) : new Array(numBodies * voices).fill(0);

    if (outputs.length < node.outlets.length) {
      node.outlets = node.outlets.slice(0, outputs.length);
    }

    const connect: number[] | undefined = node.attributes["connect"] as number[] | undefined;

    let previous = Array.isArray(connect) ? _args[connect[1]] : null;

    let indexCounter = 0;
    const _voices = mode === "inputs" ? voices : 1;
    let numArgs = 0;
    for (let i = 0; i < _voices; i++) {
      const ARGS: number[] = [];
      for (let _body of body as Statement[]) {
        let a = parseArguments(_body);
        let _numArgs = Array.from(
          new Set(a.map((b) => ((b as Statement[])[0] as CompoundOperator).value)),
        ).length;
        for (let arg of a) {
    ...
    for (let invocation = 0; invocation < voices; invocation++) {
      ...
      if (node.attributes["inputs"] !== undefined) {
        let inputs = Array.isArray(node.attributes["inputs"])
          ? (node.attributes["inputs"] as number[])
          : [node.attributes["inputs"] as number];
      ...
      for (let i = 0; i < __args.length; i++) {
        if (node.attributes[`in${i + 1}`] !== undefined) {
          let inputs = Array.isArray(node.attributes[`in${i + 1}`])
            ? (node.attributes[`in${i + 1}`] as number[])
            : [node.attributes[`in${i + 1}`] as number];
          if (!inputs.includes(invocation)) {
            __args[i] = 0;
          }
      ...
      let ret = [{ name: "call", value: invocation }, body, ...__args];
