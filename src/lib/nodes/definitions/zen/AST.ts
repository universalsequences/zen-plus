...
`;
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    let functions = blocks[i].node
      ? traverseBackwards(blocks[i].node).filter((x) => (x as ObjectNode).name === "function")
      : [];
...
export const _printStatement = (
  ...
  if (_name === "modeling.synth") {
    ...
    let trig = _printStatement(
      (statement as Statement[])[1],
      variables,
      deep + 1,
      histories,
      blocks,
    ...
    let printedModels = components.map((x) =>
      printPhysicalModel(x, variables, deep, histories, blocks),
    );
    let modelIndex = components.indexOf(component);
    return printModelConnect(
      printedModels,
      variables,
      trig,
      component,
  ...
  if (exactArgs) {
    ...
    (_name === "rawSumLoop" || _name === "sumLoop") &&
    (operator as CompoundOperator).range
  ) {
    ...
    _name === "param" &&
    (zobject.attributes["onchain"] || zobject.storedParameterValue !== undefined)
  ) {
    ...
    output = `${_name} ("${name}", ${printDeep(deep)}${statements
      .filter((x) => x !== undefined)
      .map((x) => _printStatement(x as Statement, variables, deep + 1, histories, blocks))
  ...
  .join(",\n" + printDeep(deep))}${finalArgs})`;
    ...
    output = `defun("${custom.variableName}", ${custom.value},
                ${printDeep(deep)}${_statements
                  .filter((x) => x !== undefined)
                  .map((x) =>
                    _printStatement(x as Statement, variables, deep + 1, histories, blocks),
                  )
                  .join(",\n" + printDeep(deep))}${finalArgs})`;
...
export const compileStatement = (
  statement: Statement,
  _api = api,
  _simpleFunctions = simpleFunctions,
): UGen | (() => string | number) => {
  let _compiled = _compileStatement(
    statement,
    undefined,
    undefined,
    undefined,
  ...
  if (isSimpleFunction(zenOperator, _simpleFunctions)) {
    ...
    if (name === "phasor") {
      output = phasor(
        ...
        compiledArgs[1] as Arg,
        compoundOperator.params as AccumParams,
      );
    } else if (name === "accum") {
      output = accum(
        compiledArgs[0] as Arg,
      ...
      output = round(
        compiledArgs[0] as Arg,
        compiledArgs[1] as Arg,
        compoundOperator.params as RoundMode,
      );
    } else if (name === "poke") {
      ...
      return simdDot(
        compoundOperator.block1!,
        compoundOperator.block2!,
        compiledArgs[0] as Arg,
        compiledArgs[1] as Arg,
      );
      ...
      if (modelComponent && modelComponents) {
        for (let lazyComponent of modelComponents) {
          ...
          if (component === undefined) {
            ...
            lazyComponent.component = new Component(
              _material as any,
              web,
              lazyComponent === modelComponents[0],
            );
          }
        }
        let i = 0;
        for (let lazyComponent of modelComponents) {
          ...
          if (
            component &&
            lazyComponent.connection &&
            !component.connections.some(
              (x) => lazyComponent.connection && x.component === lazyComponent.connection.component,
            )
          ) {
        ...
        if (modelComponent && modelComponent.component) {
          ...
          output = s(
            ...
            ...modelComponents.flatMap((c) =>
              c.component ? [c.component.currentChannel, c.component.prevChannel] : [],
            ),
      ...
      if (components.length === 1) {
        ...
        output = s(
          ...
          mix(
            components[0].gen(compiledArgs[0] as UGen),
            components[1].gen(add(0)),
            (compiledArgs[1] as UGen) || 0,
          ),
        );
      }
    }
...
export const getZenOperator = (
  operator: Operator,
  _api: API = api,
): ZenFunction | OnchainFunction => {
  let operatorName: string = (operator as CompoundOperator).name || (operator as string);
...
const printPhysicalModel = (
  ...
  let pitch = _printStatement(material.pitch, variables, deep + 1, histories, blocks);
  let release = _printStatement(material.release, variables, deep + 1, histories, blocks);
  let x = _printStatement(material.x, variables, deep + 1, histories, blocks);
  let y = _printStatement(material.y, variables, deep + 1, histories, blocks);
  let noise = _printStatement(material.noise, variables, deep + 1, histories, blocks);
