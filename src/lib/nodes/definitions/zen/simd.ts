...
doc("simdMatSum", {
  numberOfInlets: 3,
  inletNames: ["matrix", "inputs", "outputs"],
  numberOfOutlets: 1,
  description: "4 inputs based on matrix",
});

export const zen_simdMatSum = (object: ObjectNode, inputs: Lazy, outputs: Lazy) => {
  return (matrix: Message): Statement[] => {
    const operator = {
      name: "simdMatSum" as Operator,
      block1: matrix,
      block2: inputs(),
      block3: outputs(),
    } as CompoundOperator;
    const op = [operator] as Statement;
    op.node = object;
    return [op];
  };
};
...
export const zen_simdDotSum = (object: ObjectNode, weights: Lazy) => {
  return (matrix: Message): Statement[] => {
    console.log("simd mat sum =", matrix, weights());
    let operator = {
      name: "simdDotSum" as Operator,
      block1: matrix,
      block2: weights(),
    } as CompoundOperator;
...
export const zen_simdDot = (object: ObjectNode, buffer2: Lazy, offset1: Lazy, offset2: Lazy) => {
  return (buffer1: Message): Statement[] => {
    let operator = {
      name: "simdDot" as Operator,
      block1: buffer1,
      block2: buffer2(),
    } as CompoundOperator;
