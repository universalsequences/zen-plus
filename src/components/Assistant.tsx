...
const Assistant = () => {
    ...
    return <div className="flex relative w-full">
        {(!showAssist && <button onClick={() => {
            setShowAssist(true);
        }} className="bg-white text-black px-2 mr-2 cursor-pointer">assist.</button>)}
        {showAssist && <div style={{ backgroundColor: "#595959c2", zIndex: 1000000 }} className="flex flex-col  w-64 p-4 items-start">
            ...
            <div className="flex flex-col w-full">
                <textarea style={{ maxHeight: 200, overflowY: "scroll" }} ref={textareaRef} className="rounded-lg pr-10 pl-2 w-full bg-zinc-800 outline-none text-xs py-4 my-2" placeholder="what do you wanna build?" value={assistText} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setAssistText(e.target.value)} />
                <button onClick={() => {
                    setLoading(true);
                    let text = assistText;
                    setAssistText("");
                ...
                }} className="absolute bottom-8 right-3 rounded-lg bg-white text-black p-1 mr-2 cursor-pointer"> {loading ? <div className="lds-ring"><div></div><div></div><div></div><div></div></div> : <ArrowUpIcon className="w-5 h-5" />}</button></div>
