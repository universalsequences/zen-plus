...
const ProjectOption: React.FC<{ head: any; loadPatchToken: (x: number) => void }> = ({
  head,
  loadPatchToken,
}) => {
  ...
  return (
    <div className="flex flex-col">
      ...
      >
        {head.tokenId && <div className="w-10 text-zinc-600">#{head.tokenId.toString()}</div>}
        <div>{head.name}</div>
        ...
        <div
          ...
          className="ml-2 flex w-12"
        >
          <CommitIcon className="mr-2" />{" "}
          {head.commits
            ? head.commits.length + 1
            : head.revisionNumber
              ? head.revisionNumber.toString()
              : 1}
        </div>
      </div>
      {opened && (
        <div className="flex flex-col ml-7 py-1 pl-1 ">
          {revisions.map((head, index) => (
            <div
              key={index}
              onClick={() => loadPatchToken(head)}
              className="flex hover:bg-zinc-300 hover:text-black p-1 cursor-pointer"
            >
              <div>{head.name}</div>
              <div className="ml-auto text-zinc-600">{getTime(head.createdAt.toDate())}</div>
              <div className="ml-2 text-zinc-400 flex w-12">
                <CommitIcon className="ml-1 w-2 mr-2" color="gray" /> {revisions.length - index}
              </div>
            </div>
          ))}
        </div>
