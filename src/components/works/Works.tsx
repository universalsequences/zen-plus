...
import { zoraTestnet, zoraSepolia, goerli, baseSepolia, zora } from "wagmi/chains";
...
const Works: React.FC<{ defaultWork?: WorkOption; setShowNav: (x: boolean) => void }> = ({
  setShowNav,
  defaultWork,
}) => {
  ...
  console.log("");
  let [chain, setChain] = useState<number | null>(defaultWork ? defaultWork.chain : zora.id); //!user ? zora.id : goerli.id);
      ...
      <div className="flex-1 flex-col flex overflow-hidden">
        <div className="w-full h-32 border-b border-b-zinc-700 flex">
          <div className="flex text-lg px-10 my-auto">
            {user && (
              <div
                onClick={() => setChain(goerli.id)}
                className={
                  (chain === goerli.id ? "text-white" : "text-zinc-500") +
                  " mr-5 cursor-pointer hover:text-white transition-all hover:scale-105"
                }
              >
                testnet
              </div>
            ...
            {user && (
              <div
                onClick={() => setChain(zoraSepolia.id)}
                className={
                  (chain === zoraSepolia.id ? "text-white" : "text-zinc-500") +
                  " mr-5 cursor-pointer hover:text-white transition-all hover:scale-105"
                }
              >
                zora sepolia
            ...
            {user && (
              <div
                onClick={() => setChain(baseSepolia.id)}
                className={
                  (chain === baseSepolia.id ? "text-white" : "text-zinc-500") +
                  " mr-5 cursor-pointer hover:text-white transition-all hover:scale-105"
                }
              >
                base sepolia
            ...
            <div
              onClick={() => setChain(zora.id)}
              className={
                (chain === zora.id ? "text-white" : "text-zinc-500") +
                " mr-5 cursor-pointer hover:text-white transition-all hover:scale-105"
              }
            >
              zora
        ...
        <div className="flex flex-wrap flex-1 items-start content-start overflow-y-scroll content-start">
          {works
            .filter((x) => (!chain || x.chain === chain) && x.image)
            .map((x, i) => (
              <Work key={i} select={() => setSelectedWork(x)} work={x} />
            ))}
        </div>
      </div>
    </Skeleton>
  ...
  return (
    ...
    >
      ...
      <div className="flex pt-4 px-2 pb-2">
        ...
        {work.chain === goerli.id && (
          <div className="text-zinc-400 text-sm ml-auto italic">testnet</div>
        )}
      </div>

      <div className="flex text-xs text-zinc-300">
        <div className="px-2">
          <div>{work.price} ETH</div>

          <div className="text-center text-zinc-400 text-xs">price</div>
        </div>
        <div className="ml-auto px-2">
          <div>{trunc(work.ownerAddress)}</div>
          <div className="text-center text-zinc-400 text-xs">author</div>
        </div>
      </div>
    </div>
  );
