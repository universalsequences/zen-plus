...
import { SearchOption, Options } from "./Options";
...
import { PatchesExplorer } from "./PatchesExplorer";
...
const SearchWindow: React.FC<{ hide: () => void }> = ({ hide }) => {
      ...
      let innerPatches = node.subpatch
        ...
        .filter(
          (x) =>
            x.subpatch &&
            (text === "" ||
              (x.subpatch.name && x.subpatch.name!.toLowerCase().includes(text.toLowerCase()))),
        );
  ...
  const [option, setOption] = useState(SearchOption.Objects);
  ...
  return (
    <div
      ...
      onClick={() => {
        hide();
        setOption(SearchOption.Objects);
        setPatchOpened(null);
      }}
      style={{
    ...
    >
      <div
        ...
        className={
          (option === SearchOption.Files ? "w-2/3 h-2/3" : "w-1/2 h-1/2") +
      ...
      >
        ...
        {patchOpened && (
          <div className="flex w-full relative h-20 p-3">
            ...
            {fileExpanded && revisions && (
              <div className="absolute bottom-5  ml-10 text-zinc-400">revisions</div>
            )}
            {loading ? (
                ...
                >
                  {revisions.map((x, i) => (
                    ...
                    >
                      ...
                      {x.screenshot && <img src={x.screenshot} className="h-full w-full" />}
                    </div>
                  ))}
                </div>
              )
