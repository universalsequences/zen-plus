...
const Audio: React.FC<{ objectNode: ObjectNode }> = ({ objectNode }) => {
    ...
    return <div
        ...
        {objectNode.attributes["external-URL"] !== "" ?
            (editing ? <input onKeyDown={onKeyDown} value={text} onChange={onChange} type="text" className="overflow-hidden bg-zinc-900 p-2" /> : <div className="overflow-hidden bg-zinc-900 p-2" onClick={() => setEditing(true)} >{objectNode.attributes["external-URL"]}</div>) :
            (buffer ? <div className="bg-zinc-900 p-2 overflow-hidden">
