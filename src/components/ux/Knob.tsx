...
const Knob: React.FC<{ objectNode: ObjectNode }> = ({ objectNode }) => {
    ...
    return (<div ref={ref} style={{ width: size.width, height: size.height }} className="w-full relative flex knob-container" onMouseDown={handleMouseDown}>
        <svg viewBox="0 0 100 100" width={size.width} height={size.height} onMouseDown={handleMouseDown} style={{ cursor: 'pointer' }}>
            <path d={getPath(startAngle, 360 + 170)} stroke="#383838" strokeWidth="5" fill="none" />
            <path d={getPath(startAngle, endAngle)} stroke="red" strokeWidth="5" fill="none" />
            <line x1="50" y1="50" x2="50" y2="90" stroke="#383838" strokeWidth="5" transform={`rotate(${angle}, 50, 50)`} />
