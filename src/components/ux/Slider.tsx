...
const Slider: React.FC<{ objectNode: ObjectNode }> = ({ objectNode }) => {
  ...
  let [height, setHeight] = useState(
    objectNode.arguments[0] !== undefined ? (objectNode.arguments[0] as number) * 100 : 0.5,
  );
  ...
  const handleMouseMove = (e: any) => {
    if (editing && ref.current) {
      ...
      const newHeight = Math.min(
        Math.max(0, ((e.clientY - sliderRect.top) / sliderRect.height) * 100),
        100,
      );
      let h = 100 - newHeight;
  ...
  return React.useMemo(() => {
    return (
      <div
        ref={ref}
        style={{ backgroundColor: "#1b1a1a", width: size.width, height: size.height }}
