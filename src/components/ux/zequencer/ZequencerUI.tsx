...
export const ZequencerUI: React.FC<{ objectNode: ObjectNode }> = memo(
  ({ objectNode }) => {
    const { presentationMode, updateSize } = usePosition();
    useSelection();

    // Memoize size extraction
    const nodeSize = useMemo(
      () => objectNode.size || { width: 200, height: 200 },
      [objectNode.size],
    );
    ...
    // Memoize attributes to prevent re-computation
    const attributes = useMemo(() => objectNode.attributes, [objectNode.attributes]);
    ...
    // Memoize current step number extraction
    const currentStepNumber = useMemo(
      () => (Array.isArray(value) ? (value[0] as number) : 0),
      [value],
    );
    ...
    useEffect(() => {
      ...
      const validSelectedSteps = selectedStepsRef.current.filter(
        (step) => node.steps && findStepInPolyphonicStructure(step, node.steps),
      );
    ...
    // Use refs to access current state in event listener without dependencies
    ...
    // Create a stable mouseup handler that doesn't change dependencies
    useEffect(() => {
      const handleMouseUp = () => {
        // Always reset all editing states immediately to prevent lingering state
        setStepEditingDuration(null);
        setStepNumberMoving(null);
        setMouseStartY(null);
        setStepMoved(false);

        // Handle selection logic using refs to avoid dependency changes
        const currentSelection = selectionRef.current;
        const currentNode = nodeRef.current;

        if (currentSelection && currentNode?.steps) {
          // Get the first voice from each step position in the selection range
          const selectedVoices: GenericStepData[] = [];

          for (let i = currentSelection.fromStepNumber; i <= currentSelection.toStepNumber; i++) {
            if (i >= 0 && i < currentNode.steps.length) {
              const voices = currentNode.steps[i];
    ...
    }, [setSelectedSteps]); // Only depend on setSelectedSteps which should be stable

    // Add additional safety to clear editing state when mouse leaves the component
    const handleMouseLeave = useCallback(() => {
      // Clear all editing states when mouse leaves the entire component
      setStepEditingDuration(null);
      setStepNumberMoving(null);
      setMouseStartY(null);
      setStepMoved(false);
    ...
    // Memoize the expensive rows calculation
    const rows = useMemo((): GenericStepData[][] => {
      const rowsResult: GenericStepData[][] = [];
      const steps = node?.steps;

      if (!steps) return rowsResult;
      ...
      for (let i = 0; i < flatSteps.length; i += 16) {
        rowsResult.push(flatSteps.slice(i, i + 16));
      }
    ...
    // Memoize schema and attribute extractions
    const schema = useMemo(() => node?.stepsSchema as StepDataSchema, [node?.stepsSchema]);
    const [parameter, setParameter] = useState(schema?.[0]?.name);

    const attributeFlags = useMemo(
      () => ({
        showParameters: attributes.parameters,
        showPatternLengthControls: attributes.showPatternLengthControls,
        showParametersToggleIcon: attributes.showParametersToggle,
        pianoRollHeight: Number(attributes.pianoRollHeight || 60),
        stepOnColor: attributes.stepOnColor as string,
        stepOffColor: attributes.stepOffColor as string,
    ...
    const toggleShowParameters = useCallback(() => {
      objectNode.setAttribute("parameters", !showParameters);
    }, [objectNode, showParameters]);
    ...
    return (
      <div className="bg-zinc-900 p-2" style={{ width, height }} onMouseLeave={handleMouseLeave}>
        <div
                ...
                >
                  ...
                  >
                    <div
                      ...
                      style={{
                        background: `linear-gradient(90deg, #00000000, ${stepOnColor})`,
                        width:
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      prevProps.objectNode.size === nextProps.objectNode.size &&
      prevProps.objectNode.attributes === nextProps.objectNode.attributes
    );
  },
);

ZequencerUI.displayName = "ZequencerUI";
