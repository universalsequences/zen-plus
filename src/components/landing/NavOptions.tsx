...
const NavOptions = () => {
    ...
    return (
        <div className="flex flex-col">
            {navOption === NavOption.Files ? <Option name="Files" option={NavOption.Files} /> : <Option name="Home" option={NavOption.Home} />}
...
const Option: React.FC<{ option: NavOption, name: string }> = ({ name, option }) => {
    ...
    return (<div
        ...
        className={(navOption === option ? "text-zinc-100" : "text-zinc-500") + " mr-10 cursor-pointer hover:text-white transition-colors"}>{name}</div>);
};
