...
const PositionedComponent: React.FC<PositionedComponentProps> = ({
  ...
  useEffect(() => {
    ...
    if (!isCustomView && ref.current && !(node as ObjectNode).attributes["Custom Presentation"]) {
      ...
      const isResizableNodeType = objectNode.isResizable || 
   ...
   */
  const startResizing = useCallback((e: React.MouseEvent<HTMLDivElement>, orientation: Orientation) => {
    e.stopPropagation();
  ...
  const onMouseDown = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (ref.current) {
      ...
      const objectNode = node as ObjectNode;
      const isZenCustom = objectNode.name === "zen" && 
  ...
  const onClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const hasSelectHandler = (node as ObjectNode).subpatch?.objectNodes.some(x => x.name === "onPatchSelect");
  ...
  return React.useMemo(() => {
    ...
    const shouldSkipOverflow = skipOverflow || 
    ...
    if (objectNode.attributes["Custom Presentation"] && nodeSize) {
      ...
      allowSize = true;
    } else if (objectNode.name === "zen" && nodeSize) {
      allowSize = true;
      nodeStyle.width = nodeSize.width;
      nodeStyle.height = nodeSize.height;
    }
