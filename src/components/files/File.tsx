...
const FileComponent: React.FC<{ isRevision: boolean, className: string, openFile: (x: File | null) => void, fileExpanded: File | null, setFileExpanded: (x: File | null) => void, file: File, setFileToOpen: (x: any | null) => void, setRevisions: (x: File[]) => void }> = ({ file, setFileToOpen, setRevisions, fileExpanded, setFileExpanded, openFile, className, isRevision }) => {
    ...
    return (
        <div
            ...
            }} className={(file === fileExpanded ? "bg-zinc-700 rounded-lg " : "") + "flex flex-col m-3 text-sm border-zinc-800 border hover:border-zinc-200 transition-all p-5 cursor-pointer relative"}>
            <div className={`${className}  mb-5 hover:bg-zinc-700 transition-colors bg-zinc-800 rounded-lg relative overflow-hidden`}>
                {file.screenshot && <img src={file.screenshot} className="w-full h-full object-cover" />}
                {file.commits && <div
                    onClick={(e: any) => { e.stopPropagation(); setRevisions([]); setFileExpanded(file) }}
                    style={file.screenshot ? { backgroundColor: "#ffffffa3", backdropFilter: "blur(9px)", color: "#3d3d3d" } : {}}
