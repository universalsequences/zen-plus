...
const WriteOnchain: React.FC<{ setTokenId: (x: number | null) => void, previousTokenId: number, isSubPatch?: boolean, name: string, compressed: string }> = ({ compressed, name, isSubPatch, previousTokenId, setTokenId }) => {
    ...
    const { config } = usePrepareContractWrite({
        ...
        args: [DROP_CONTRACT, account ? account.address : "", name, compressed, isSubPatch ? true : false, previousTokenId],
        value: parseEther("0")
    })
    ...
    useEffect(() => {
        if (data && !transactionError && !transactionLoading) {
            ...
            fetchTokenId(publicClient, data.hash as any).then(
                (x: any) => {
                    setTokenId(x);
                });
        }
...
export async function fetchTokenId(publicClient: PublicClient, transactionHash: `0x{ string }`): Promise<number | null> {
