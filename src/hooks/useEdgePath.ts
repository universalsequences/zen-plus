...
export const useEdgePatch = (node: ObjectNode, outletNumber: number, connection: IOConnection) => {
  ...
  if (slotview_source) {
    ...
    source_h = SLOT_VIEW_HEIGHT;
  }
  if (slotview_dest) {
    dest_w = SLOT_VIEW_WIDTH;
    dest_h = SLOT_VIEW_HEIGHT;
  }
  return React.useMemo(() => {
    if (sourceCoordinate && destCoordinate) {
      ...
      if (slotview_source) {
        ...
        source_height = SLOT_VIEW_HEIGHT;
      }

      let sourceBetween = (width - 10) / Math.max(1, numOutlets - 1);
      let dest_width = (connection.destination as ObjectNode).size
        ? (connection.destination as ObjectNode).size!.width
        : source_w;

      let dest_height = node.size ? node.size.height : SLOT_VIEW_HEIGHT;
      if (slotview_dest) {
        dest_width = SLOT_VIEW_WIDTH;
        dest_height = SLOT_VIEW_HEIGHT;
      }
      ...
      let d =
        connection.segmentation !== undefined && !isStraight
          ? generateSegmentedPaths(
              sourceCoordinate,
              destCoordinate,
              (connection.source as ObjectNode).position.y,
      ...
      let point1 = cubicBezier(0.1, points[0], points[1], points[2], points[3]);
      let point2 = cubicBezier(0.9, points[0], points[1], points[2], points[3]);
...
const generateSegmentedPaths = (
  ...
  let path3 = [cx2 - offsetX1, segmentY, _x2, segmentY, _x2, segmentY, _x2, y2];
  if (isSmol) {
    path3 = [_x2 + offsetX, segmentY, _x2, segmentY, _x2, segmentY, _x2, y2];
  }
