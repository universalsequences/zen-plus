...
export const usePositionStyle = (node: Identifiable & Positioned, _isCustomView?: boolean) => {
  ...
  let nodePosition = isCustomView
    ? node.presentationPosition || node.position
    : coordinates[node.id];
  const prevMode = useRef<boolean>(presentationMode);

  const positionStyle = useMemo(() => {
    let _nodePosition = isCustomView
      ? node.presentationPosition || node.position
      : presentationMode
        ? node.presentationPosition || node.position || coordinates[node.id]
        : node.position || coordinates[node.id];
    if (_nodePosition) {
      let style: any = {
        left: _nodePosition.x + "px",
        top: _nodePosition.y + "px",
        zIndex: node.zIndex === -1 ? 0 : (node.zIndex as number) + 1,
      };
