...
export const useSubPatchLoader = (objectNode: ObjectNode) => {
    ...
    const loadSubPatch = useCallback(async (x: File) => {
        ...
        if (serializedSubPatch) {
            ...
            const getIO = (node: ObjectNode, name: string, iolet: IOlet, iolets: IOlet[]): string | null => {
                if (!node.subpatch) {
                    return null;
                }
                let ioletNumber = iolets.indexOf(iolet);
                let ioNode = node.subpatch.objectNodes.find(x => x.name === name && x.arguments[0] === ioletNumber + 1);
                if (!ioNode) {
                    return null;
                }
            ...
            const getAdjacentNodes = (iolets: IOlet[], isInput: boolean): ObjectNode[] => {
                return iolets.flatMap(
                    iolet => iolet.connections
                        .filter(x => undefined !== (isInput ? (x.source as ObjectNode).subpatch : (x.destination as ObjectNode).subpatch))
                        .map(x => isInput ? x.source : x.destination)) as ObjectNode[];
            };

            let inputNodes = getAdjacentNodes(objectNode.inlets, true);
            let outputNodes = getAdjacentNodes(objectNode.outlets, false);

            const disconnectIO = (node: ObjectNode, iolets: IOlet[], isInput: boolean): ConnectionSignature[] => {
                return iolets.flatMap(
                    x => x.connections.map(
                        y => {
                            let ret: ConnectionSignature = {
                                node: isInput ? y.source : y.destination,
                                inletNumber: isInput ? objectNode.inlets.indexOf(y.destinationInlet) : y.destination.inlets.indexOf(y.destinationInlet),
                                outletNumber: isInput ? y.source.outlets.indexOf(y.sourceOutlet) : objectNode.outlets.indexOf(y.sourceOutlet),
                                io: getIO(objectNode, "in", y.destinationInlet, objectNode.inlets) ||
                                    getIO(y.source as ObjectNode, "out", y.sourceOutlet, y.source.outlets) || "other"
                            };
            ...
            const getMatchingIO = (ioType: string, node: ObjectNode, name: string): ObjectNode[] => {
                ...
                return node.subpatch.objectNodes.filter(x => x.name === name && x.attributes.io === ioType);
            };
                ...
                for (let connection of x) {
                    ...
                    for (let node of nodes) {
                        ...
                        if (node.subpatch && objectNode.subpatch && io) {
                            ...
                            if (!isInput) {
                                ...
                                let exactMatches = matchingIONodes.filter(x => ((x.arguments[0] as number) - 1) === outletNumber);
                                ...
                                let exactMatches = matchingIONodes.filter(x => ((x.arguments[0] as number) - 1) === inletNumber);
                                let secondaryMatches = matchingIONodes.filter(x => {
                                    let ionumber = ((x.arguments[0] as number) - 1)
                    ...
                    if (!foundMatch && io) {
                        ...
                        if (isInput) {
                            ...
                            let inlet = objectNode.inlets[inletNumber];
                            let ioNode = (objectNode.subpatch!).objectNodes.find(x => x.name === "in" && x.arguments[0] === inletNumber + 1);
                            if (ioNode) {
                            ...
                            // we havent found anything so just find any outlet that matches the type of this inlet
                            let outlet = objectNode.outlets[outletNumber];
                            let ioNode = (objectNode.subpatch!).objectNodes.find(x => x.name === "out" && x.arguments[0] === inletNumber + 1);
                            if (ioNode) {
                                io = ioNode.attributes.io as string;
                            }
            ...
            const reconnectIO2 = (isInput: boolean) => {
                ...
                for (let i = 0; i < iolets.length; i++) {
                    let iolet = iolets[i];
                    let ioNode = (objectNode.subpatch!).objectNodes.find(x => x.name === (isInput ? "in" : "out") && x.arguments[0] === i + 1);
                    if (!ioNode) {
                        continue;
                    }
